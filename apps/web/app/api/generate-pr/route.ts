...
export async function POST(req: Request) {
  ...
  // Resolve a GitHub auth context for the session repo. This supports users
  // without a linked GitHub account by falling back to a GitHub App
  // installation token, and refreshes short-lived App tokens that may have
  // expired since the sandbox was created.
  const resolvedAuth =
    sessionRecord.repoOwner && sessionRecord.repoName
      ...
      : null;
  if (sessionRecord.repoOwner && sessionRecord.repoName && !resolvedAuth) {
    return Response.json(
  ...
  const fetchResult = await withTemporaryGitHubAuth(
    sandbox,
    resolvedAuth?.token,
    () =>
      sandbox.exec(
